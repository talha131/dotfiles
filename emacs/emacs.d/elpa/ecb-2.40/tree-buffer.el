...
`trebubuffer-spec'. It always returns a copy of the registered string in
  ...
  (let* ((node-points (save-excursion
    ...
    (if (tree-node->expanded node)
        (let ((exp-node-children-count (1+ (tree-buffer-count-subnodes-to-display node)))
          ...
          ;; the window (= condition 1) and if not all of itýs children are
          ...
          (if (and (save-excursion
                   ...
                   (not (save-excursion
                          ...
                          (pos-visible-in-window-p (point) window))))
              ;; optimize the display of NODE and itýs children so as much as
