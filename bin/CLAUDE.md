...
- **Naming:** user scripts are prefixed `my-*` (e.g. `my-transcribe`,
  `my-reddit-to-json`). No file extension. **Exceptions:** `gh` and `claude`
  deliberately omit the prefix because they *shadow* the real binaries (PATH
  order puts this dir ahead of Homebrew) rather than adding new commands.
